...
It may be helpful to think of _every_ `m2` object as being a missing value (`NA`, a `logical(1)`) with two M2 attributes: their name (`m2_name`) and a capture-all named list called `m2_meta`. These can be accessed with `m2_name()` and `m2_meta()`.  For example, a ring, having no analogous object in R, is a `NA` with attributes:
```{r objects_2}
r <- ring(c("x","y"), "QQ")
str(r)
class(r)
